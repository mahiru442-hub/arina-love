...
/* First-letter Continue button: reveal after opening and pin it BELOW the actual paper. */
(function(){
  const button=document.getElementById('introContinue');
  const envelope=document.getElementById('envelope');
  const letter=document.getElementById('letter');
  if(!button || !envelope || !letter) return;
  ...
  function revealAfterOpen(){
    ...
    revealTimer=setTimeout(()=>{
      ...
      placeContinue();
      button.classList.add('after-open-visible');
    },4200);
  }

  envelope.addEventListener('click',revealAfterOpen,{once:true});
  window.addEventListener('resize',()=>{
    if(button.classList.contains('after-open-visible')) requestAnimationFrame(placeContinue);
  },{passive:true});
  window.addEventListener('orientationchange',()=>{
    setTimeout(()=>{ if(button.classList.contains('after-open-visible')) placeContinue(); },180);
  },{passive:true});
  window.addEventListener('hashchange',()=>{
